...
ARTIFACTS_DIR = AIRFLOW_HOME / "artifacts" / "food"
...
def scale_frame(frame):
    df = frame.copy()
    X, y = df.drop(columns=["calories"]), df["calories"]
...
def train():
    ...
    mlflow.set_experiment("linear model food")
    ...
    with mlflow.start_run():
        ...
        y_pred_food = power_trans.inverse_transform(y_pred.reshape(-1, 1))
        y_val_food = power_trans.inverse_transform(y_val)

        rmse, mae, r2 = eval_metrics(y_val_food, y_pred_food)
        ...
        joblib.dump(best, ARTIFACTS_DIR / "food_model.pkl")
        joblib.dump(scaler, ARTIFACTS_DIR / "scaler.pkl")
        joblib.dump(power_trans, ARTIFACTS_DIR / "power_transformer.pkl")

        print(f"[OK] Saved model to: {ARTIFACTS_DIR / 'food_model.pkl'}")
